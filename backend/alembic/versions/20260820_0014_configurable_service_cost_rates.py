...
def upgrade() -> None:
    ...
    with op.batch_alter_table("service_rate_cards") as batch:
        ...
        batch.add_column(
            sa.Column(
                "personnel_operating_rate", sa.Numeric(18, 4), server_default="0", nullable=False
            )
        )
        batch.add_column(
            sa.Column(
                "personnel_standby_rate", sa.Numeric(18, 4), server_default="0", nullable=False
            )
        )
        batch.add_column(
            sa.Column("other_rate", sa.Numeric(18, 4), server_default="0", nullable=False)
        )
        batch.create_foreign_key(
            op.f("fk_service_rate_cards_hole_section_id_hole_sections"),
            "hole_sections",
            ["hole_section_id"],
            ["id"],
            ondelete="RESTRICT",
        ...
        batch.create_check_constraint(
            "valid_service_rate_basis",
            "rate_basis IN ('daily','per_service','per_section','fixed')",
        )
        batch.drop_constraint("non_negative_service_rates", type_="check")
        batch.create_check_constraint(
            "non_negative_service_rates",
            "operating_rate >= 0 AND standby_rate >= 0 AND mobilisation_rate >= 0 AND demobilisation_rate >= 0 AND personnel_operating_rate >= 0 AND personnel_standby_rate >= 0 AND other_rate >= 0",
        )
    ...
    sections = connection.execute(
        sa.text(
            "SELECT DISTINCT hole_section FROM service_rate_cards "
    ...
    for section in sections:
        ...
        if section_id is None:
            ...
            connection.execute(
                sa.text("INSERT INTO hole_sections (id, code, name) VALUES (:id, :code, :name)"),
                {"id": section_id, "code": code, "name": str(section).strip()},
            )
        connection.execute(
            sa.text(
                "UPDATE service_rate_cards SET hole_section_id = :id WHERE hole_section = :section"
            ),
    ...
    op.create_index(
        op.f("ix_service_rate_cards_hole_section_id"), "service_rate_cards", ["hole_section_id"]
    )
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("service_rate_cards") as batch:
        ...
        batch.create_check_constraint(
            "non_negative_service_rates",
            "operating_rate >= 0 AND standby_rate >= 0 AND mobilisation_rate >= 0 AND demobilisation_rate >= 0",
        )
