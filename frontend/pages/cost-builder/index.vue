...
async function load() { const [e, r, c] = await Promise.all([api.list(), reqApi.listAfes(undefined, 'submitted'), master.list('currencies')]); estimates.value = e.items; submittedAfes.value = r.items; currencies.value = c.items.filter(x => x.is_active) }
