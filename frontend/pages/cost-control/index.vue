...
function duplicateRow(row: GridRow) { rows.value.push({ ...row, _key: ++key }) }
function applyPaste() { const parsed = parseTsv(pasteText.value, pasteColumns); rows.value.push(...parsed.map(item => ({ ...blank(), ...item, external_transaction_id: item.external_transaction_id || null, vendor_code: item.vendor_code || null, quantity: item.quantity || null, unit_code: item.unit_code || null }))); pasteVisible.value = false; pasteText.value = '' }
