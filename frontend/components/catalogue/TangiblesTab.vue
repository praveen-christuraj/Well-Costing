...
<template>
  <div class="tab-panel">
    <ExcelGrid
      ...
      :duplicate-key-fields="['manufacturer', 'unit_rate_po', 'cost_uplift', 'description']"
      paste-hint="Code and Final Cost are auto; Scope/Category/Subcategory/Manufacturer/UOM/Currency dropdowns are excluded from paste. Duplicate names are allowed when Manufacturer, Rate as per PO, Uplift or Description differ. New dropdown values can be added with the Manage buttons."
    ...
    <ImportDialog
      ...
      hint="Headers: tangible_name, tangible_scope (Drilling/Completion/Others), category, subcategory, manufacturer (new values auto-created), po_number, uom, unit_rate_po, cost_uplift (default 100), currency, effective_date (flexible), description, remarks. Each subcategory is linked to the row's category — unknown subcategories are created under it. Codes auto-generate. Duplicate names are allowed when Manufacturer, Rate as per PO, Uplift or Description differ; re-importing a name identical on all of those refreshes the existing row."
